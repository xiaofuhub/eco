$PBExportHeader$w_qa_reviewer_assignment_view.srw
$PBExportComments$Reviewer Assignment - Reviewer View
forward
global type w_qa_reviewer_assignment_view from w_response
end type
type cb_refresh from commandbutton within w_qa_reviewer_assignment_view
end type
type st_1 from statictext within w_qa_reviewer_assignment_view
end type
type dw_assignment from u_pics_dw within w_qa_reviewer_assignment_view
end type
type cb_exit from commandbutton within w_qa_reviewer_assignment_view
end type
type cb_update from commandbutton within w_qa_reviewer_assignment_view
end type
type cb_clear from commandbutton within w_qa_reviewer_assignment_view
end type
end forward
...
integer width = 5010
integer height = 2760
string title = "Reviewer Assignment - Reviewer View"
boolean minbox = true
boolean maxbox = true
...
global w_qa_reviewer_assignment_view w_qa_reviewer_assignment_view
...
on w_qa_reviewer_assignment_view.create
int iCurrent
call super::create
...
on w_qa_reviewer_assignment_view.destroy
call super::destroy
...
IF Len(trim(inv_qa.of_qaauthority(ls_user)))= 0  THEN
	Messagebox('Error','Access Denied.Please check the QA Authority Code')
	close(this)
END IF
...
//
//	Description:
//	Refresh list if other than QC0 approved
//
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//	
//	Revision History
...
string text = "A default number of books will be displayed Default to be determined by QAS"
...
integer height = 2272
integer taborder = 40
string dataobject = "d_qa_reviewer_assignment_view"
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//
openwithparm(w_pics_retrieve_msg_box,"Refreshing data , Please Wait...")
end event
...
//									
// Murali K.			02/13/2008      005 PICS Modifications	 Reqs: QAS 6.2
// Murali K.			07/22/2008		new view vw_qa_reviewer_assign/ inserts moved to this code
...
IF dw_assignment.ModifiedCount( ) > 0 THEN
	...
	IF ll_rc > 0 THEN
		FOR li_loop = 1 	TO ll_rc
			...
			ls_check_box = dw_assignment.object.check_box[li_loop]
			IF ls_check_box = 'Y' THEN // new record for qa reviewer assignment
			...
			UPDATE QA_REVIEWER_ASSIGNMENT
				...
				WHERE BKSEQ = :ll_bkseq and bkmed = 'DB' and active_status_code = 'A' using sqlservertrans ;
	...
	END IF
		IF ib_update THEN
			MessageBox("Update","Books assigned to Reviewer.",Information! )
			dw_assignment.Retrieve() // refresh queue
		ELSE
...
IF ll_rc > 0 THEN
	FOR li_loop = 1 	TO ll_rc
		ls_check_box = dw_assignment.object.check_box[li_loop]
		IF ls_check_box = 'Y' THEN 
			dw_assignment.object.check_box[li_loop] = 'N'
			dw_assignment.SetItemStatus(li_loop, 0,  Primary!, DataModified!)
