$PBExportHeader$w_qa_product_review_blind_new.srw
forward
global type w_qa_product_review_blind_new from w_sheet
end type
type cb_add_row from commandbutton within w_qa_product_review_blind_new
end type
type mle_comments from uo_mle within w_qa_product_review_blind_new
end type
type cb_spell from commandbutton within w_qa_product_review_blind_new
end type
type cb_updateweb from u_cb within w_qa_product_review_blind_new
end type
type cb_add_narr from u_cb within w_qa_product_review_blind_new
end type
type cb_comments from u_cb within w_qa_product_review_blind_new
end type
type st_4 from statictext within w_qa_product_review_blind_new
end type
type cb_find from u_cb within w_qa_product_review_blind_new
end type
type cb_exit from u_cb within w_qa_product_review_blind_new
end type
type cb_clear from u_cb within w_qa_product_review_blind_new
end type
type cb_update from u_cb within w_qa_product_review_blind_new
end type
type st_1 from statictext within w_qa_product_review_blind_new
end type
type em_bkno from uo_conno within w_qa_product_review_blind_new
end type
type dw_qa_prod_review from u_pics_dw within w_qa_product_review_blind_new
end type
type dw_qa_qastg from u_pics_dw within w_qa_product_review_blind_new
end type
type dw_qa_narr from u_pics_dw within w_qa_product_review_blind_new
end type
type cb_narr from u_cb within w_qa_product_review_blind_new
end type
type st_3 from statictext within w_qa_product_review_blind_new
end type
type dw_qa_recagcy from u_pics_dw within w_qa_product_review_blind_new
end type
type dw_qa_cntr from u_pics_dw within w_qa_product_review_blind_new
end type
end forward
...
integer width = 2601
integer height = 1352
string title = "Quality Assurance (Product Review with windows eye)"
...
global w_qa_product_review_blind_new w_qa_product_review_blind_new
...
on w_qa_product_review_blind_new.create
int iCurrent
call super::create
...
on w_qa_product_review_blind_new.destroy
call super::destroy
...
if bjaws = TRUE then
	...
	of_SetJaws(FALSE)
	MessageBox("ERROR in windows eyes","Windows Eyes must be running.")
end if
  			...
  			IF ll_rows > 0 THEN
				// retrieve the recorded agency(prdr) from narrtbl
					...
					ELSEIF TRIM(Luserid) = "mich" THEN
						ll_rows = dw_qa_qastg.Retrieve(Lbkseq,ls_cntr,'1','4')
					ELSE
						MessageBox("ERROR","Only Tom Bickford or Debbie Brown can use this screen.")
						RETURN
					END IF
					IF ll_rows = 0 THEN
...
IF Lqa_approve = 'R' OR Lqastatcd = 'B' THEN
	...
	IF Lqastg = '1' AND Luserid = 'tbic' THEN
			
	// Qastg is 1 and this user is tom bickford
	...
	ELSEIF Lqastg = '4' AND Luserid = 'tbic' THEN
	...
	ELSEIF Lqastg='2' AND Luserid = 'dabro' THEN
			
	// Qastg is 1 and this user is Debbie brown
	// cusros on qacompdt
		dw_qa_qastg.object.bkseq[ll_rc]=Lbkseq
		dw_qa_qastg.object.bkmed[ll_rc]=Lbkmed
		dw_qa_qastg.object.cntr[ll_rc]=Lcntr
	...
	ELSEIF Lqastg='1' AND Luserid = 'mich' THEN
			
	// Qastg is 1 and this user is Mike
		dw_qa_qastg.object.bkseq[ll_rc]=Lbkseq
		dw_qa_qastg.object.bkmed[ll_rc]=Lbkmed
		dw_qa_qastg.object.cntr[ll_rc]=Lcntr
		dw_qa_qastg.object.ttl[ll_rc]=Lttl
	...
	ELSEIF Lqastg='4' AND Luserid = 'mich' THEN
...
ELSEIF Lqa_approve = 'A' AND Lmed = 'RTB' AND Lqastg = '1' THEN
	...
	END IF
ELSE
	MessageBox("ERROR","You may only add rows if the book is rejected or put on hold. Recorded Talking Books RTB can have QC1 and QC4.")
END IF
...
event itemfocuschanged;call super::itemfocuschanged;IF dwo.name = "bkseq" THEN
...
event itemfocuschanged;call super::itemfocuschanged;inv_jaws.of_read('contract number',FALSE)
end event
