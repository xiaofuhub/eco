...
// #2214 changes validate only version
li_cnt = sqlservertrans.F_AUTH_PICSUSER( as_version) 
...
IF sqlservertrans.sqlcode <> 0  THEN
	IF sqlservertrans.sqldbcode <> 0 THEN
			CHOOSE CASE sqlservertrans.sqldbcode
				...
				CASE 20002
					ls_msg = 'Invalid Software Version! You must upgrade your machine to '+  vs_ver
...
ELSE
	IF ls_sha1_yn = 'N' OR Isnull(ls_sha1_yn) OR Len(Trim(ls_sha1_yn)) = 0 THEN 
		...
		SELECT F_HASH(:as_password) INTO :ls_server_pwd FROM PICSUSER WHERE USERID = :as_userid using sqlservertrans ;
...
of_SetVersion ("Version 5.0")
//of_SetVersion ("Version 5.0.01")
//
...
// Murali K. 06/24/2009 #2214 4.2 Password encryption and user id validation
//////////////////////////////////////////////////////////////////////////////
...
//SQLServerTrans.DBParm = "StaticBind=0,DisableBind=1"
//SQLserverTrans.userid = as_userid
//SQLserverTrans.dbpass = as_password

// appeon
SQLServerTrans.DBMS = "O90 Oracle9i (9.0.1)"
SQLServerTrans.LogPass = "pcsadmin" // Tracker 2120 12/1/2008
SQLServerTrans.LogId = "pcsadmin"
SQLserverTrans.userid = as_userid
SQLserverTrans.dbpass = as_password
SQLServerTrans.AutoCommit = False
SQLServerTrans.DBParm = "Cachename='pcsp',StaticBind=0,DisableBind=1"
////appeon
...
//SQLserverOracleTrans.DBParm = "StaticBind=0,DisableBind=1"
...
SQLserverOracleTrans.LogId = "picadmin"
SQLserverOracleTrans.AutoCommit = False
SQLserverOracleTrans.DBParm = "Cachename='picp',StaticBind=0,DisableBind=1" // give a common name to cache
//// appeon

// jdbc
//// Profile PCST
//sqlservertrans.DBMS = "JDBC"
//sqlservertrans.LogPass = 'pcsadmin'
//sqlservertrans.LogId = "pcsadmin"
//sqlservertrans.AutoCommit = False
//sqlservertrans.DBParm = "URL='jdbc:oracle:thin:pcsadmin/pcsadmin@rsora11gt.loctest.gov:1521:pcst',Driver='oracle.jdbc.driver.OracleDriver'"
//
//// Profile PICT
//sqlserveroracletrans.DBMS = "JDBC"
//sqlserveroracletrans.LogPass = 'picadmin'
//sqlserveroracletrans.LogId = "picadmin"
//sqlserveroracletrans.AutoCommit = False
//sqlserveroracletrans.DBParm = "Driver='oracle.jdbc.driver.OracleDriver',URL='jdbc:oracle:thin:picadmin/picadmin@rsora11gt.loctest.gov:1521:pict'"
...
IF SQLserverTrans.sqlcode <> 0 THEN
   ...
   Else                                             //check for other error messages
		ls_message = "Unable to Connect to PICS Internal Database. "
...
ELSE // DB login is successful
   ...
   // Get the current version from the user application	
	...
	// #2214 06/25/09 use this function only to validate version
...
//	END IF
	...
	// 06/24/2009 #2214 Version 4.2	user id and password validation
...
//	// Successful logon to pcsadmin, load the messages for db error
	...
	gnv_app.inv_error.of_SetLogFile("picserrlog.txt")
	...
	IF SqlServerOracleTrans.sqlcode <> 0 THEN
		IF SqlServerOracleTrans.SqlDbcode = -951 THEN            //check for invalid userid
			MessageBox("Login Error","Invalid User ID/Password for Oracle PICS Web Database.",StopSign!)
			Return -1	
		ELSEIF SqlServerOracleTrans.SqlDBcode = -952 THEN       //check for invalid password
			MessageBox("Login Error","Invalid User ID/Password for Oracle PICS Web Database.",StopSign!)
			Return -1
		Else                                             //check for other error messages
			MessageBox("Database Connection Error","Unable to Connect to Oracle using PICS Web Database." +& 
