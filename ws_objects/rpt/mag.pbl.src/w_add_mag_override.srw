$PBExportHeader$w_add_mag_override.srw
forward
global type w_add_mag_override from w_response
end type
type dw_override from u_dw within w_add_mag_override
end type
type cb_exit from u_cb within w_add_mag_override
end type
type cb_update from u_cb within w_add_mag_override
end type
end forward
...
global w_add_mag_override w_add_mag_override
...
integer height = 936
integer taborder = 10
string dataobject = "d_mag_override"
end type
...
IF DWO.name = "ovrcd" THEN
	IF wf_check_duplicates(Loverride) = TRUE THEN
	  ...
	  dw_override.SetItem(row,"ovrcd",Null_string)
	  RETURN 1
    END IF
...
dw_override.AcceptText()

li_max_rows = dw_override.RowCount()

//Delete blank lines from the datawindow
FOR li_loop = li_max_rows TO 1 step -1
	ls_override = dw_override.GetItemString(li_loop,"ovrcd")
...
IF dw_override.ModifiedCount() > 0 THEN
		...
		IF li_rtn_code = 1 THEN
			...
			w_magazine_invoice_tracking.dw_invoice_tracking_rc.GetChild ("maginv_ovrcd", ldwc_ovrd)
