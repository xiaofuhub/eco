$PBExportHeader$w_create_files.srw
forward
global type w_create_files from w_main
end type
type st_1 from statictext within w_create_files
end type
type dw_ds_pgm85_distsched_bklist_rowcnt from u_pics_dw within w_create_files
end type
type dw_dist from u_pics_dw within w_create_files
end type
type dw_distsched_inx from u_pics_dw within w_create_files
end type
type st_transupdate from statictext within w_create_files
end type
type dw_sp1_distsched_orl from u_pics_dw within w_create_files
end type
type sle_batch_date from singlelineedit within w_create_files
end type
type uo_progress from u_progressbar within w_create_files
end type
type cb_cancel from commandbutton within w_create_files
end type
type cb_go from commandbutton within w_create_files
end type
end forward
...
integer width = 1394
integer height = 888
string title = "Create Distribution Schedule"
...
global w_create_files w_create_files
...
String ls_msgtext[ ] = {" 0 %","10 %", "20 %", "30 %", "40 %", "50 %", "60 %", &
	...
	SetNull(ls_dsflag)
//	li_bound= upperbound(w_distribution_schedule.ii_bkseq[])
...
ll_rows = dw_distsched_inx.RowCount()
	...
	ELSe
		...
		ELSE
			...
			//now is time to update dw_dsdtdsflag in w_distribution_schedule for mchar table
			li_re=w_distribution_schedule.dw_dsdtdsflag.RowCount()
			ldt_cur=datetime(today(),now())
			IF f_check_dberror(sqlserveroracletrans,"distsched at SP9") THEN
				IF rtn=1 THEN
					rtn=w_distribution_schedule.dw_dsdtdsflag.update()
					if rtn=1 then
						w_distribution_schedule.st_dist.text=&
												string(ldt_cur,'mm/dd/yyyy hh:mm:ss')
						w_distribution_schedule.cbx_distsched.checked=true
	...
	if lb_done=true then
		w_distribution_schedule.ib_web_done=true
	//	closewithreturn(w_create_files,'Y')
	else
		w_distribution_schedule.ib_web_done=false
	//	closewithReturn(w_create_files,'N')
	end if
	close(this)
end subroutine
...
event retrieveend;call super::retrieveend;w_create_files.SetRedraw(TRUE)
w_distribution_schedule.SetRedraw(TRUE)
...
w_distribution_schedule.dw_dist.SetTransObject(SqlServerTrans)
IF w_distribution_schedule.cbx_distsched.checked=FALSE THEN
...
IF w_distribution_schedule.cbx_file.checked =FALSE THEN
	 w_distribution_schedule.cbx_file.enabled=TRUE
	 
	 w_distribution_schedule.dw_dist.object.distfile[1]=ldt_cur
	 w_distribution_schedule.dw_dist.object.distfilerows[1]=i_distfilerows
	 w_distribution_schedule.dw_dist.object.distfilelib[1]=i_libs
	 w_distribution_schedule.dw_dist.object.distfilebooks[1]=i_books
	 w_distribution_schedule.dw_dist.object.distprdr[1]=i_prdr
	 w_distribution_schedule.dw_dist.object.distfileuser[1]=ls_usid
	 rtn=w_distribution_schedule.dw_dist.Update()
	 IF rtn=1 THEN	
		COMMIT USING SqlServerTrans;
		w_distribution_schedule.cbx_file.checked =TRUE
	 ELSE		
		ROLLBACK USING SqlServerTrans;
		RETURN
	 END IF
...
IF w_distribution_schedule.cbx_distsched.checked =TRUE THEN
	Messagebox('Distribution Schedule','Distribution schedule has been created on the web.')
	RETURN
END IF
w_distribution_schedule.cbx_distsched.enabled=TRUE
...
IF ll_rows = 0 THEN
	Messagebox("ERROR","No rows found for distribution."+&
		"~nPlease contact system administrator.")
ELSE
	parent.SetRedraw(TRUE)
	uo_progress.visible=TRUE
...
//		dw_sp1_distsched_orl.TriggerEvent('ue_addrow')
		...
		dw_sp1_distsched_orl.SetItem(li_cur,'distsched_type_code', 'R') // regular schedule
...
//	w_create_files.SetRedraw(TRUE)
	...
	IF f_check_dberror(SQLserverOracleTrans,"distsched") THEN
		IF rtn=1 THEN
			rtn=w_distribution_schedule.dw_dsdtdsflag.Update()
			IF rtn=1 THEN					
				w_distribution_schedule.dw_dist.object.distsched[1]=ldt_cur
				w_distribution_schedule.dw_dist.object.distschedrows[1]=li_cur
				w_distribution_schedule.dw_dist.object.mcharupdrows[1]=li_re
				w_distribution_schedule.dw_dist.object.distscheduser[1]=ls_usid
				rtn=w_distribution_schedule.dw_dist.Update()
				IF rtn=1 THEN
					// Now lets do some cleaning up and remove some of the old records from distsched table.
					Close(w_pics_retrieve_msg_box)	
					w_distribution_schedule.st_dist.text = String(ldt_cur,'mm/dd/yyyy hh:mm:ss')
					w_distribution_schedule.cbx_distsched.checked=TRUE
					
					
					// 04/02/2008 Update batch_data table after creating distribution schedule
				...
				ELSE
					...
					ROLLBACK USING SQLserverOracleTrans;
					ROLLBACK USING SqlServerTrans;
					IF IsValid(w_create_files) THEN
						closewithreturn(w_create_files,'N')
					ELSE
						RETURN
					END IF
