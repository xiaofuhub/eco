$PBExportHeader$w_distribution_schedule.srw
forward
global type w_distribution_schedule from w_sheet
end type
type cb_deselect from u_cb within w_distribution_schedule
end type
type dw_bklist from u_pics_dw within w_distribution_schedule
end type
type dw_dist from u_pics_dw within w_distribution_schedule
end type
type st_dist from statictext within w_distribution_schedule
end type
type st_file from statictext within w_distribution_schedule
end type
type st_imp from statictext within w_distribution_schedule
end type
type dw_batch_process from u_pics_dw within w_distribution_schedule
end type
type cbx_distsched from u_cbx within w_distribution_schedule
end type
type cbx_file from u_cbx within w_distribution_schedule
end type
type cbx_imp from u_cbx within w_distribution_schedule
end type
type dw_dsdtdsflag from u_pics_dw within w_distribution_schedule
end type
type dw_import_cp_allotbatch_sql from u_pics_dw within w_distribution_schedule
end type
type st_insertupdate from statictext within w_distribution_schedule
end type
type rb_batch from u_rb within w_distribution_schedule
end type
type rb_include from u_rb within w_distribution_schedule
end type
type st_bkorrow from statictext within w_distribution_schedule
end type
type sle_book# from u_sle within w_distribution_schedule
end type
type cb_new from u_cb within w_distribution_schedule
end type
type cbx_include from u_cbx within w_distribution_schedule
end type
type em_schdate from u_em within w_distribution_schedule
end type
type st_1 from statictext within w_distribution_schedule
end type
type cb_include from u_cb within w_distribution_schedule
end type
type cb_cancel from u_cb within w_distribution_schedule
end type
type cb_find from u_cb within w_distribution_schedule
end type
type dw_update_selcd_sched from u_dw within w_distribution_schedule
end type
type uo_progress from u_progressbar within w_distribution_schedule
end type
type cb_print from u_cb within w_distribution_schedule
end type
type gb_progress from groupbox within w_distribution_schedule
end type
type cbx_import from u_cbx within w_distribution_schedule
end type
type st_include from statictext within w_distribution_schedule
end type
type cb_file from u_cb within w_distribution_schedule
end type
type cb_create from u_cb within w_distribution_schedule
end type
type cb_import from u_cb within w_distribution_schedule
end type
type dw_no_master from u_dw within w_distribution_schedule
end type
type dw_distribution_schedule from u_dw within w_distribution_schedule
end type
type dw_import_cp_allotbatch from u_dw within w_distribution_schedule
end type
end forward

global type w_distribution_schedule from w_sheet
integer width = 2757
string title = "Distribution Schedule"
...
global w_distribution_schedule w_distribution_schedule

type variables
str_distrib_schedule istr_add
string is_add='N', is_cntr1[],is_cntr2[], is_bkmed[]
long i_count=0, i_remainder, ii_bkseq[]
datetime id_cabdt[], id_assigndt[], id_cabdate
boolean ib_web_done=false
end variables
...
on w_distribution_schedule.create
int iCurrent
call super::create
...
on w_distribution_schedule.destroy
call super::destroy
...
//
//	Description:
//	Batch needs to be closed to do dist sched
//
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//	
//	Revision History
...
// 04/08/2008 Can distribute only if batch is closed
//setnull(ld_cabdt)
//select cabdt
//into :ld_cabdt
...
//	Messagebox("", "Current Batch is not closed.Cannot proceed with Distribution Schedule")
//	RETURN
//END IF
//////////////
...
String ls_repeat, ls_cld,ls_date, ls_bkno_list[],ls_bkmed, ls_bkno, ls_dsflag,ls_cascd,&
		ls_rtn, ls_cntr2, ls_sum_bkno, ls_null, ls_cntrtype
		
Date ld_dsdt, ld_cabdt
DateTime ld_dsdt_dt
str_distrib_schedule lstr
...
// 10/29/2008 update order qty with actual qty before schedule creation

//ld_cabdt = wf_get_cabdt()
//
//update sched a
//set a.ordqty = (select actual_qty from batch@pic_link
...
OpenWithParm(w_create_files, lstr)
...
// Murali K.			04/08/2008		Batch need to be closed to import
// Murali K.			11/10/2008 		check cascd from mchar for bkseq and bkmed jointly
//											update cascd with 'P' for each bkseq and bkmed not globally
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//
...
li_re=Messagebox('','This process will import the copy allotment quantities.~nDo you wish to proceed?',exclamation!,yesNo!,1)
IF li_re= 2 THEN
	RETURN
END IF
//***this must remember to uncomment and comment
Open(w_gets_batch_date)
lstr=message.powerObjectParm
ld_cabdt= lstr.ld_date
ld_cabdt_dt=DateTime(ld_cabdt,Time('00:00:00'))
//ld_cabdt=date('08/01/2001')
//id_cabdate= ld_cabdt
//***this must remember to uncomment and comment
ls_ok_cancel= lstr.arraymed[1]
IF ls_ok_cancel='CANCEL' THEN
	RETURN
END IF
ls_bk#old=''
J=0
...
// Since we do not transfer LIBDEF to DEF at copy allotment process anylonger, this part of the code needs to be commented out. 5/31/2005
//IF IsNull(ldt_defimp) OR IsNull(ldt_schedins)  OR &
//	IsNull(ldt_webupd)  THEN
//	Messagebox('Error','The batch formation process for this copy allotment batch'+&
//	'~nwas not successfully completed. Can not continue.')
//	IF IsValid(w_pics_retrieve_msg_box) THEN
//		Close(w_pics_retrieve_msg_box)
...
IF IsNull(ldt_distsched)=FALSE THEN
	...
	cbx_distsched.checked=TRUE
END IF// check the last step i.e. insert rows into distsched in oracle
//11/06/2008 commented because they can import normal or special distribution at different stages for a batch date
//IF IsNull(ldt_batchimp)=FALSE THEN
//	Messagebox('Error','You have already imported this batch on '+&
//		'~n'+String(ldt_batchimp,'mm/dd/yyyy hh:mm:ss'))	
//
//	IF IsValid(w_pics_retrieve_msg_box) THEN
//		Close(w_pics_retrieve_msg_box)
//	END IF
...
IF li_row_count<=0 THEN
	Messagebox('Error','No rows found to import. '+&
...
//dw_import_cp_allotbatch.object.t_defqty.visible=true
FOR i=1 TO li_row_count
...
//	li_selqty=dw_import_cp_allotbatch.object.selqty[i]
	...
	li_selqty=dw_import_cp_allotbatch.object.actual_qty[i]
	...
	IF li_bkseq<>li_bkseqold THEN
		...
		//	Close(w_pics_retrieve_msg_box)
		//	RETURN
			CONTINUE // unsupported apppen feature goto replaced 3/24/2010
		END IF
		li_bkseq=lds_cascd.object.bkseq[1]
		ls_cascd=lds_cascd.object.cascd[1]
		
		//11/10/2008 check cascd from mchar for bkseq and bkmed jointly
		select cascd
		into :ls_cascd
		from mchar
		where bkseq = :li_bkseq and bkmed = :ls_bkmed using sqlservertrans ;
		///////
...
//			lds_cascd.object.cascd[1]='P'
	...
	IF lb_skip =FALSE THEN//if lb_skip=true do not update for this bkseq,bkmed
		...
		// 11/10/2008 actual qty might be zero and it has to be imported 
...
//		END IF
		...
		// 11/10/2008 update for each book not globally
		UPDATE mchar
		set cascd='P', dsdt=NULL, dsflag=NULL 
		where cabdt= :ld_cabdt_dt and bkseq = :li_bkseq  and bkmed = :ls_bkmed
		USING SqlServerTrans;
...
//wf_distrib_schedule_icd_no_icd()
st_bkorrow.visible=FALSE
sle_book#.visible=FALSE
uo_progress.visible=FALSE
st_insertupdate.text=''
...
integer height = 908
integer taborder = 70
string dataobject = "d_distribution_schedule_null_no_master"
...
integer height = 920
integer taborder = 40
string dataobject = "d_distribution_schedule"
boolean hscrollbar = true
boolean hsplitscroll = true
boolean livescroll = false
boolean ib_isupdateable = false
...
integer height = 920
integer taborder = 60
string dataobject = "d_import_cp_allotbatch"
boolean hscrollbar = true
boolean hsplitscroll = true
boolean livescroll = false
boolean ib_isupdateable = false
