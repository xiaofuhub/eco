$PBExportHeader$w_voucher_retrieve_update_new.srw
forward
global type w_voucher_retrieve_update_new from w_sheet
end type
type st_voucher from statictext within w_voucher_retrieve_update_new
end type
type cb_exit from u_cb within w_voucher_retrieve_update_new
end type
type cb_update from u_cb within w_voucher_retrieve_update_new
end type
type cb_select_contract from u_cb within w_voucher_retrieve_update_new
end type
type cb_print_all from u_cb within w_voucher_retrieve_update_new
end type
type dw_detail_voucher from u_dw within w_voucher_retrieve_update_new
end type
type dw_voucher_retrieve from u_dw within w_voucher_retrieve_update_new
end type
type dw_detail_voucher_pcsdt from u_dw within w_voucher_retrieve_update_new
end type
end forward
...
global w_voucher_retrieve_update_new w_voucher_retrieve_update_new
...
on w_voucher_retrieve_update_new.create
int iCurrent
call super::create
...
this.Control[iCurrent+6]=this.dw_detail_voucher
this.Control[iCurrent+7]=this.dw_voucher_retrieve
this.Control[iCurrent+8]=this.dw_detail_voucher_pcsdt
end on

on w_voucher_retrieve_update_new.destroy
call super::destroy
...
integer taborder = 0
integer textsize = -10
string text = "&Create Voucher No(s)"
end type

event clicked;call super::clicked;Integer rtn,vno_seq,li_rows,i
...
integer taborder = 0
integer textsize = -10
string text = "Create PDF(s) and Print  &All"
end type
...
IF dw_detail_voucher_pcsdt.visible= TRUE THEN
	...
	ELSE
		...
		ls_pdf_filename = "p:\vouchers\"+ls_vno+".pdf"
		...
		IF pdf_created = FALSE THEN  
			...
			dw_detail_voucher_pcsdt.Object.DataWindow.Export.PDF.Method = XSLFOP!
			rtn = dw_detail_voucher_pcsdt.SaveAs(ls_pdf_filename, PDF!, FALSE)
//			rtn = dw_detail_voucher_pcsdt.SaveAs(ls_pdf_filename, PDF!, FALSE)
			IF rtn <> 1 THEN
				Messagebox("ERROR"," Error in creating the PDF file for voucher number "+ls_pdf_filename)
			ELSE
				...
				IF f_check_dberror(SQLServerTrans,"Voucher") THEN	
					...
					COMMIT USING SQLServerTrans;
					Messagebox("UPDATE","Update successfull. The PDF(s) are stored in p:\vouchers directory")
					RETURN 1	
				ELSE
					Messagebox("UPDATE","Update failed contact system administrator.")
					ROLLBACK USING SQLServerTrans;
...
ELSEIF dw_detail_voucher.visible= TRUE THEN
	...
	ELSE
		...
		IF pdf_created = FALSE THEN 
			 ...
			 // Create and place the PDFs
			dw_detail_voucher.Object.DataWindow.Export.PDF.Method = XSLFOP!
			rtn = dw_detail_voucher.SaveAs(ls_pdf_filename, PDF!, FALSE)
//			rtn = dw_detail_voucher.SaveAs(ls_pdf_filename, PDF!, FALSE)
			IF rtn <> 1 THEN
				Messagebox("ERROR"," Error in creating the PDF file for voucher number "+ls_pdf_filename)
			ELSE
...
ELSEIF dw_voucher_retrieve.visible = TRUE THEN
	...
	// Go through the loop and print, create PDF and update database
	FOR i = 1 TO dw_voucher_retrieve.RowCount()
		...
		ELSE
			...
			IF rtn > 0 THEN
				...
				// After print the datawindow, check to see if the PDF is created and the flag is not NULL.
...
integer height = 1088
integer taborder = 20
string dataobject = "d_voucher_contract_update_xls"
...
w_voucher_retrieve_update_new.windowState = maximized!
...
integer height = 1088
integer taborder = 10
string dataobject = "d_voucher_contract_update_pcsdt_xls"
boolean hscrollbar = true
boolean vscrollbar = false
boolean border = false
boolean livescroll = false
