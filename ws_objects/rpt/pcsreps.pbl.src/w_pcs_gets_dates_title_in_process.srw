$PBExportHeader$w_pcs_gets_dates_title_in_process.srw
forward
global type w_pcs_gets_dates_title_in_process from w_response
end type
type cb_ok from u_cb within w_pcs_gets_dates_title_in_process
end type
type cb_cancel from u_cb within w_pcs_gets_dates_title_in_process
end type
type em_stdt from u_em within w_pcs_gets_dates_title_in_process
end type
type em_enddt from u_em within w_pcs_gets_dates_title_in_process
end type
type st_1 from u_st within w_pcs_gets_dates_title_in_process
end type
type st_2 from statictext within w_pcs_gets_dates_title_in_process
end type
end forward
...
string Title="Titles in process by author"
...
global w_pcs_gets_dates_title_in_process w_pcs_gets_dates_title_in_process

on w_pcs_gets_dates_title_in_process.create
int iCurrent
...
on w_pcs_gets_dates_title_in_process.destroy
...
IF NOT ISDATE(em_stdt.Text) or NOT ISDATE(em_enddt.text) Then
		MessageBox("Error","Expecting catalog date and actual end dates!")
		em_stdt.SetFocus()
...
Else
	...
	CloseWithReturn(w_pcs_gets_dates_title_in_process,lstr_cds_report)	
End IF
