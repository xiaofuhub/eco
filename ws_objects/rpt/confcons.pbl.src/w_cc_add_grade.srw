$PBExportHeader$w_cc_add_grade.srw
forward
global type w_cc_add_grade from w_response
end type
type dw_cc_grade from u_pics_dw within w_cc_add_grade
end type
type cb_exit from u_cb within w_cc_add_grade
end type
type cb_update from u_cb within w_cc_add_grade
end type
end forward
...
dw_cc_grade dw_cc_grade
cb_exit cb_exit
cb_update cb_update
end type
global w_cc_add_grade w_cc_add_grade
...
on w_cc_add_grade.create
int iCurrent
call super::create
this.dw_cc_grade=create dw_cc_grade
this.cb_exit=create cb_exit
this.cb_update=create cb_update
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.dw_cc_grade
this.Control[iCurrent+2]=this.cb_exit
this.Control[iCurrent+3]=this.cb_update
end on

on w_cc_add_grade.destroy
call super::destroy
destroy(this.dw_cc_grade)
...
li_max_rows = dw_cc_grade.RowCount()

//Delete blank lines from the datawindow
FOR li_loop = li_max_rows TO 1 step -1
	ls_grade_code = dw_cc_grade.object.gradecd[li_loop]
	IF IsNull(ls_grade_code) THEN
		dw_cc_grade.DeleteRow(li_loop)
	END IF
NEXT
