...
public function integer wf_checkusers (string l_username);string ls_userid
 ...
 SELECT NLuser.userid
    INTO :ls_userid
    FROM NLuser  
   WHERE NLuser.userid = :l_username
...
event closequery;///////456123//456/////456////455/////456///////达到////////456///////////////////////////////////////
//
//	Event:  closequery 123
//
//	Description:
...
//m_nl_main.m_edit.m_deleterow.Enabled = FALSE
//m_nl_main.m_edit.m_addrow.enabled = FALSE
...
ls_userid=gnv_app.of_getuserid() // trim(SQLserverTrans.userid)
...
//       ls_passwordold = lds2.object.passwrd[1]
		...
		// 02/26/2009 validate 
		select passwrd,passwrdchg, admin_passwrdchg
		into :ls_passwordold, :ld_passwordchg, :ld_admin_date
		from NLUSER
		where userid = :ls_userid using sqlservertrans ;
		
		IF f_check_dberror(SqlServerTrans,"Selecting new password and passwrdchg from NLUSER failed")=FALSE THEN
			RETURN 1
		END IF
	...
	ELSEIF is_password <> ls_passwordold THEN
		...
		IF ll_rows > 0  AND li_counter > 0 THEN
			...
			IF li_counter >= 3 THEN
				...
				li_counter++
				ldt_today = datetime(Today(),Now())
				IF w_eco_main.password_must_change THEN
...
//						WHERE userid = :ls_userid
//						USING SqlServerTrans;
//						IF f_check_dberror(SqlServerTrans,"Updating NLUSER for user "+ls_userid)=FALSE THEN
//							ROLLBACK USING SqlServerTrans;
//						ELSE
   ...
   WHERE sha1_yn = 'N' using sqlservertrans ;
	...
	ELSE
		 ...
		 from
		  ...
		  where passwrd = :check_password OR passwrd = trim(:data) using sqlservertrans ;
	END IF
	...
	ELSE
		...
		ELSE
			...
			MessageBox('Status','Password Update successful')
			 w_eco_main.password_must_change = FALSE
