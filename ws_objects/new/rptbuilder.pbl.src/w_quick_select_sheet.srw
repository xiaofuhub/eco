...
If Handle(w_quick_select) < 1 then
	...
	///////////////////////////////////////////////////////////////////////////////////
	ll_start = PosA(lower(ls_criteriaselectstmt)," where ")
	...
	//check if this occurs, if so, this is the end of the where clause
		ll_end = PosA(lower(ls_criteriaselectstmt)," order by ")

		If ll_end = 0 Then 	// no order by
			ll_end = LenA (ls_criteriaselectstmt)
		End If

		ls_whereclause = MidA ( ls_criteriaselectstmt, ll_start, ll_end - ll_start +1)
	...
	///////////////////////////////////////////////////////////////////////////////////
	ll_start = PosA(lower(ls_criteriaselectstmt)," order by ")
	...
	If ll_start > 0 Then
		...
		ll_end = PosA(ls_criteriaselectstmt, " ", ll_start)
		do while ll_end > 0
			If Lower(MidA(ls_criteriaselectstmt, ll_start, 1)) = "a" or  &
			   Lower(MidA(ls_criteriaselectstmt, ll_start, 1)) = "d"  Then
				//This is the asc or desc stmt, just copy it
				ll_rc = PosA (ls_criteriaselectstmt, ",", ll_end) + 1   // adjust for space after order sequence 
				If ll_rc > 1 Then ll_end = ll_rc    //no need to adjust if this was the last asc/desc 
				ls_orderclause = ls_orderclause + MidA(ls_criteriaselectstmt, ll_start, ll_end - ll_start + 1)
		      Else  			// must be a column # --- convert it to a column name
				ls_column = MidA (ls_criteriaselectstmt, ll_start, ll_end - ll_start +1)
...
If Handle(w_quick_select) < 1 then
	...
	ll_start_update = PosA(lower(ls_criteriaselectstmt),"update ")
	ll_start_delete = PosA(lower(ls_criteriaselectstmt),"delete ")
